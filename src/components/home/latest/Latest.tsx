...
import latestData from './latestdata'; // Assuming latestData doesn't have an 'id' field
    ...
    <div className="md:container md:wrapper px-[.8rem]">
          ...
          {tags.map((item, index) => {
            return (
              <div key={index}> {/* Using index as the key here */}
                <div
                  className="text-lg cursor-pointer hover:text-pink hover:font-semibold"
                  onClick={() => {
                    setActive(item.btn);
                  }}
                >
      ...
      <div className="">
        <div className="md:grid grid-cols-3 gap-8">
          {latestData
            .filter((e) => e.category === active) // Filters by active category
            .slice(0, 6) // Limits to 6 items
            .map((item, index) => {
              return (
                <div key={index} className="h-[306px] grid mb-20"> {/* Using index as the key here */}
                  ...
                  <div className="flex justify-between items-end text-navy">
                    <h4 className="border-b-2">{item.info}</h4>
